...
def mega_sena():
    ...
    loteria = Loteria(quantidade_numeros=parameters["quantidade_numeros"],
                      quantidade_apostas=parameters["quantidade_jogos"])
        ...
        while len(loteria.apostas) < parameters["quantidade_jogos"]:
            ...
            if parameters["fazer_previsao"]:
                ...
                if predict >= parameters["taxa_classificacao"]:
                    ...
                    apostaCandidataRepository.cadastrar_aposta_candidata(loteria.aposta_candidata,
                                                                         f"{float(predict):.2f}")
            else:
                loteria.apostas.append(loteria.aposta_candidata)
                esperados.append(loteria.aposta_candidata)
